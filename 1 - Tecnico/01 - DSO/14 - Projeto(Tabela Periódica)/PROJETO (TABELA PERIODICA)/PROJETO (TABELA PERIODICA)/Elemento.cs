...
namespace PROJETO__TABELA_PERIODICA_
{
    class Elemento
    {
        ...
        public Elemento(string nome, int nroAtomico)
        {
            ...
            comando.CommandText = "SELECT * FROM ELEMENTOS WHERE SIMBOLO = ?s or NRO_ATOMICO = ?n";
            
            comando.Parameters.AddWithValue("?s", nome);
            comando.Parameters.AddWithValue("?n", nroAtomico);
