...
{
 	$db = mysql_connect(); 
	mysql_select_db('net_presence_bloglike'); 	
}
else
*/
{
	       ...
	       if (mysql_select_db($reserved_sql_database)==false)
	       {
	       		if ($reserved_sql_can_create_database==1)
			{
		       		if (createDatabase($reserved_sql_database))
		       		{
		       			if (mysql_select_db($reserved_sql_database)==false)
		       			{
		       				setError("Can't select database '".$reserved_sql_database."'");
		       			}
		       		}
		       		else
		       		{
		       			setError("Error creating SQL database '".$reserved_sql_database."'");
		       		}				
			}				
...
function doRequestRetrieveComments($tbl_comment_name,$id_article)
{
	...
	if ($req)
	{
		while($data = mysql_fetch_assoc($req)) 
		{
			...
			$author = filterText($data['author']);
			$comment = filterText($data['comment']);
			$email = filterText($data['email']);
			echo "<OBJECT>\n";
...
if ($result=="false")
{
	...
	echo "<error val=\"".$s_error_message."\">";	
